        ...
        VisBuff.Canvas.Pen.Color := PenColor;
         for i := 0 to 128 do begin
           ...
           if YPos >= FFTFallOff[i] then FFTFallOff[i] := YPos
              ...
              case DrawType of
                0 : begin
                       VisBuff.Canvas.MoveTo(X + i*4, Y + VisBuff.Height);
                       VisBuff.Canvas.LineTo(X + i*4, Y + VisBuff.Height - FFTFallOff[i]);
                       if ShowPeak then VisBuff.Canvas.Pixels[X + i*4, Y + VisBuff.Height - FFTPeacks[i]] := Pen;
                    end;
